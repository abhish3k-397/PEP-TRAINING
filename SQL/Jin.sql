...
select concat(c.first_name," ",c.last_name),o.order_id, o.order_date,p.product_name,oi.quantity, (oi.quantity * oi.unit_price) as total from customers c join orders o on c.customer_id = o.customer_id join order_items oi on o.order_id = oi.order_id join products p on oi.product_id = p.product_id;


select concat(c.first_name," ",c.last_name) as CustomerName,o.order_id, o.order_date,p.product_name,oi.quantity, (oi.quantity * oi.unit_price) as total from customers c left join orders o on c.customer_id = o.customer_id left join order_items oi on o.order_id = oi.order_id left join products p on oi.product_id = p.product_id;



select concat(c.first_name," ",c.last_name), sum(oi.quantity * oi.unit_price) as total_spent 
from customers c 
join orders o 
...
from (select sum(oi.quantity * oi.unit_price) as total_spent 
from orders o 
join order_items oi 
on o.order_id = oi.order_id
...
on o.order_id = oi.order_id
group by o.customer_id
) as avg_table) order by total_spent desc limit 1;

select state, concat(c.first_name," ",c.last_name) as CustomerName from customers c join orders o on c.customer_id = o.customer